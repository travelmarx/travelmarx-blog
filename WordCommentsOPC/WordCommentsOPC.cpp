	...
	IXMLDOMDocument2 **document)
{
	...
	if (SUCCEEDED(hr) && selectionNamespaces)
	{
		...
		if (SUCCEEDED(hr))
		{
			hr = partContentXmlDocument->setProperty(SysAllocString(L"SelectionLanguage"), v);
		}
		if (SUCCEEDED(hr))
		{
			AutoVariant v;
			...
			if (SUCCEEDED(hr))
			{
				hr = partContentXmlDocument->setProperty(SysAllocString(L"SelectionNamespaces"), v);
			}
		}
	}
	if (SUCCEEDED(hr))
